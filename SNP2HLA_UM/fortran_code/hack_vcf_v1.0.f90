      program hack_vcf
!--------------------------------------------------------------------------------------
!     Program hack_vcf is a utility program to be used as part of my modification
!     of the SNP2HLA package of Jia et al (2013).  This utility hacks allele
!     identities of the vcf-formatted reference panel output by my modified 
!     MakeReference script so it can can be used as input into Beagle 5.1 within my
!     modified SNP2HLA script.  The non-standard alleles of the .vcf reference panel 
!     (1 letter amino acid codes; P/A codes) are converted into strings of ACTG
!     alleles that are permissible under the new allele-naming restrictions of
!     Beagle 5.1 that weren't enforced by the older Beagle 3.0.4.
!
!     The phased reference input file (vcf 4.2 format) is presumably generated
!     by a previous run of my MakeReference script.  Other required input files
!     are the Plink .fam and Beagle .markers files also generated by MakeReference.
!     The Beagle .markers file has no header lines, and each row contains four
!     fields--marker ID, basepair postion, allele 1 identity, and allele 2 indentity.  
!     The marker IDs may be up to 100 characters in length; allele IDs may be up 
!     to 100 characters in length and may use any character in the standard ASCII 
!     set.  The user is also asked to supply the chromosome ID for the input
!     markers.
!
!     For allele hacking, the program first identifies all sets of markers that 
!     are identical for both basepair position and ref/alt alleles (i.e., amino 
!     acid and 2-digit or 4-digit HLA alleles with same position and P/A alleles,
!     or a microarray HLA SNP and its MakeReference-computed SNP-dictionary equivalent,
!     or very rarely an AA and HLA SNP with same position and happenchance matching
!     of 1 letter AA and nucleotide codes).  Markers within these identical
!     position/allele sets are then artificially distinguished as distinct markers 
!     by appropriately changing the ref/alt alleles.  Duplicate sets with up to 4 
!     markers are converted to all possible unique combinations of ref=A and alt=AX 
!     alleles where X is all 4 permutations of A/C/T/G, sets with 5-16 markers are 
...
!     where XXX is all 64 permutations of A/C/T/G, sets with 65-256 markers are 
!     converted to ref=A and alt=AXXXX where XXXX is all 256 permutations of 
!     A/C/T/G, and sets with 257-1024 markers are converted to ref=A and alt=AXXXXX
!     where XXXXX is all 1024 permutations of A/C/T/G.  After duplicated markers
!     are made distinguishable, the program then checks if any markers have alleles 
!     that aren't a combination of A/C/T/G or the missing character code 0 (usually 
!     amino acid positions with only two possible residues).  These, if any, are 
!     converted to A/AA alleles.  Because this conversion can create new sets of 
!     apparently identical markers, particularly for HLA-DRB3/4/5 AA and SNP variants 
!     with identical positions and one Z allele, another round of deduplication 
!     is necessary. 
!
...
!     fields, and a header line that includes all sample IDs.  The genotype fields
!     in the output vcf are identical to those in the input vcf; only the identities
!     of alleles with non-standard coding in the input file are changed to their
!     hacked equivalents.
!
...
!     Revision History:												 
!                                                                      
!     v 1.0  04/08/21   First version, created by reduction and modification of
!                       v1.7 of the beagle2vcf utility script.
!
!     References:
!
...
!--------------------------------------------------------------------------------------
      ...
      integer i,item,ioerr,imk,istart,iend,j,k, &
              nskip,nmarker,nheader,nsample,ndup, &
              chr,revert_opt,i_position,pos_first,pos_last
      integer,allocatable :: position(:)
      character(1) accept
      character(2) aset1(4,2)
      character(3) aset2(16,2)
      character(3), allocatable :: gtype(:)
      character(4) aset3(64,2)
      character(5) aset4(256,2)
      character(6) aset5(1024,2)
      character(6) string
      character(8) date
      character(35) answer(2),gtype_opts(2)
      character(45) ctmp
      character(200) mkid,mkid1,mkid2,a1,a2
      character(200), allocatable :: samples(:), ralleles(:,:), ralleles_original(:,:)
      character(1000) fnmarker,fnfam,fnvcf_in,fnvcf_out
      character(1001) arg
      logical exist
...
!     Initialize program parameters to default values
      ...
      fnmarker='HAPMAP_CEU_hg19_REF.markers'
      fnfam='HAPMAP_CEU_hg19_REF.fam'
      fnvcf_in='HAPMAP_CEU_hg19_REF.bgl.vcf'
      fnvcf_out='HAPMAP_CEU_hg19_REF.vcf'
      chr=6
      revert_opt=2
      answer(1)=' (1) Yes'
      answer(2)=' (2) No'
      ...
      if (command_argument_count()>0) then
         do i=1,command_argument_count(),2
           ...
           select case (arg)
             ...
             case('-fnvcf_in')
               call get_command_argument(i+1,fnvcf_in)
	           inquire(file=fnvcf_in,exist=exist)
               if (.not.exist) then
                 write(*,'(/a,a,a)') ' Input file ',trim(fnvcf_in),' not in directory'
                 stop
               end if
             case('-fnvcf_out')
               call get_command_argument(i+1,fnvcf_out)
             case('-chr')
...
!        user indicates (with a '21') that all selections have been made.
         ...
         do while (item/=21)
           ...
           nskip=14
           write(*,9110) fnmarker,fnfam,fnvcf_in,fnvcf_out,chr,answer(revert_opt)
           write(*,9190) accept

9110       format ('  1  MARKER INPUT FILE',T45,A35/ &	
                   '  2  FAMILY INPUT FILE',T45,A35/ & 
                   '  3  VCF INPUT FILE',T45,A35/ &
			       '  4  VCF OUTPUT FILE',T45,A35/ &
                   '  5  CHROMOSOME ID IN VCF OUTPUT',T45,I2/ &
                   '  6  ARRAY SNPs: KEEP OLD ALLELES (1-2)?',T45,A35)
...
9203            format (' Name of unhacked-allele vcf input file: ')
                read (*,'(a)') fnvcf_in
	            inquire(FILE=fnvcf_in,EXIST=EXIST)
	            do while (.not.exist)
	              write (*,9203,ADVANCE='NO')
...
9204            format (' Name of hacked-allele vcf output file: ')
                read (*,'(a)') fnvcf_out
              case (5)
	            write (*,9205,ADVANCE='NO') 
9205            format (' Chromosome identifier for variants in output vcf file (1-21): ' )
	            read (*,*) chr
...
9206            format (' Keep original alleles for microarray-genotyped SNPs? (1-2): ' )
	            read (*,*) revert_opt
...
!     Allocate dynamic arrays, rewind file, and read in all sample IDs
...
!     Determine number of meta-information + header lines in vcf input file (i.e., all
!     records beginning with a "#" in first column.
      
      open (unit=15,file=fnvcf_in)
      nheader=0
      do 
        read(15,*,iostat=ioerr) ctmp
        if (ctmp(1:1)/="#") exit
...
!     Open both input files and position phased genotypes file to first marker record.  
      
      open (unit=15,file=fnmarker)
      open (unit=16,file=fnvcf_in)
...
!     use of tab-delimited output in accordance with vcf 4.2 standards.
      ...
      allocate (gtype(nsample))
      
      do imk=1,nmarker

        read(15,*) mkid1,i_position,a1,a2
        read(16,*) ctmp,ctmp,mkid2,ctmp,ctmp,ctmp,ctmp,ctmp,ctmp,(gtype(i),i=1,nsample)
...
9310      format (' Marker number ',i6,' differs in two input files.  Halting program.')
        ...
        write(17,'(*(g0,:,"'//achar(9)//'"))') chr,i_position,trim(mkid1),trim(ralleles(imk,1)),trim(ralleles(imk,2)),'.','.','PR','GT',(gtype(i),i=1,nsample)
        
      end do
      
      close(15); close(16); close(17)

   end program hack_vcf